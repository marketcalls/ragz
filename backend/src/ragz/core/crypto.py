"""Envelope-encryption primitives under the external KEK (iron rule 3).

Canonical home for the AES-256-GCM primitives. This lives in `core` (the
lowest layer) so that BOTH `modules/secrets` (provider secrets) and
`core/app_settings` (the JWT signing key, RAGZ-PUB-07) can encrypt at rest
under the same KEK without violating the layered-import contract
(`ragz.core` must never import `ragz.modules`). `modules/secrets/crypto.py`
re-exports these names, so there remains exactly ONE implementation of
encrypt/decrypt in the codebase.
